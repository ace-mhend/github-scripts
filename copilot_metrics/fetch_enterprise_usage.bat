...
REM ============================================
REM Fetch Copilot Usage for Entire Enterprise
REM ============================================
REM GET /enterprises/{enterprise}/copilot/usage
REM ============================================
...
REM Validate enterprise
if "%ENTERPRISE%"=="your-enterprise-slug" (
    echo ERROR: Please set ENTERPRISE in config.bat.
    exit /b 1
)

REM Build API endpoint
set "ENDPOINT=%API_BASE_URL%/enterprises/%ENTERPRISE%/copilot/usage"
...
set "OUTPUT_FILE=%OUTPUT_DIR%\enterprise_usage_%ENTERPRISE%_%TIMESTAMP%.json"
set "TEMP_FILE=%OUTPUT_DIR%\temp_response.json"
set "ALL_DATA_FILE=%OUTPUT_DIR%\enterprise_usage_%ENTERPRISE%_all_%TIMESTAMP%.json"

echo ============================================
echo Fetching Copilot Usage for Enterprise: %ENTERPRISE%
echo ============================================
...
if not errorlevel 1 (
    type "%TEMP_FILE%"
    echo.
    echo ERROR: API returned an error message. Check your credentials and enterprise slug.
    del "%TEMP_FILE%" 2>nul
    exit /b 1
)

REM Count records in this page (rough estimate by counting timestamps or user_id)
...
copy "%TEMP_FILE%" "%OUTPUT_DIR%\enterprise_usage_%ENTERPRISE%_page!PAGE!_%TIMESTAMP%.json" >nul
...
call "%~dp0summarize_usage.bat" "%ALL_DATA_FILE%"
