#!/bin/bash
# ============================================
# Fetch Copilot Usage for Entire Enterprise
# ============================================
...
# Validate enterprise
if [[ "$ENTERPRISE" == "your-enterprise-slug" || -z "$ENTERPRISE" ]]; then
    echo "ERROR: Please set ENTERPRISE environment variable."
    exit 1
fi

# Build API endpoint
ENDPOINT="$API_BASE_URL/enterprises/$ENTERPRISE/copilot/usage"
...
while $HAS_MORE; do
    ...
    cp "$TEMP_FILE" "$OUTPUT_DIR/enterprise_usage_${ENTERPRISE}_page${PAGE}_${TIMESTAMP}.json"
...
echo "Output saved to: $OUTPUT_DIR/enterprise_usage_${ENTERPRISE}_page*_${TIMESTAMP}.json"
